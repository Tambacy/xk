# -*- coding: utf-8 -*-
"""生成应用图标（淡紫圆角方块 + 白色「选」字），多尺寸打包成 .ico。

配色与界面完全同一套：渐变两端 #8B79C9 → #4A3A85 是主色阶的亮端与暗端。
任务栏、桌面快捷方式、开始菜单、安装向导用的都是这张图，必须和界面一个颜色 ——
否则会出现「图标一个色、界面另一个色」这种脱节（上一版就发生过一次）。

弧纹和界面里的画法一致（同心椭圆），但在方块里几乎就是横贯全宽的几条直线，
会像划痕，所以图标上**不画** —— 那点装饰留给界面。
"""
...
TOP = (139, 121, 201)      # #8B79C9  主色渐变亮端
BOTTOM = (74, 58, 133)     # #4A3A85
WHITE = (255, 255, 255)
